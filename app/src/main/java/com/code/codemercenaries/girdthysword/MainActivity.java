...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (systemPreferences.getBoolean("initiated", true)) {
            ...
            settingsPreferences.edit().putInt("chunk_size",3).commit();

            for(int i=0;i<66;i++){
                indexPreferences.edit().putBoolean(bookNames[i],false).commit();
                for(int j=1;j<=numOfChap[i];j++){
                    indexPreferences.edit().putBoolean(bookNames[i]+"_"+j,false).commit();
                }
            }

            // record the fact that the app has been started at least once
            systemPreferences.edit().putBoolean("initiated", false).commit();
        }
        else{
